...
#Shipment information recieved 一个个问题检查
#Shipment information processed
...
if __name__ == '__main__':
    ...
    try:
        if len(sys.argv) < 2:
            print("perfectreport.py -h\nperfectreport.py -f <start_time> <end_time> <clientname>..\nperfectreport.py -uf <clientname>..\nperfectreport.py -a <clientname>..")
        elif sys.argv[1] == "-h":
            print("-h\t\t\t\t\t\thelp info \n-a <start_time> <end_time> <clientname>..\t从start_time到end_time的clientname的所有单\n-f <start_time> <end_time> <clientname>..\t从start_time到end_time的clientname的妥投单\n-uf <clientname>..\t\t\t\tclientname的未妥投单")
        elif sys.argv[1] == "-f":
            start_time = sys.argv[2]
            end_time = sys.argv[3]
            for clientname in sys.argv[4:]:
                exportToFinishedExcel(clientname,start_time,end_time)
        elif sys.argv[1] == "-uf":
            for clientname in sys.argv[2:]:
                exportToUnfinishedExcel(clientname)
        elif sys.argv[1] == "-a":
            start_time = sys.argv[2]
            end_time = sys.argv[3]
            for clientname in sys.argv[4:]:
                exportToExcel(clientname,start_time,end_time)
        else:
            print("perfectreport.py -h\nperfectreport.py -f <start_time> <end_time> <clientname>..\nperfectreport.py -a <start_time> <end_time> <clientname>..\nperfectreport.py -uf <clientname>..")
    except IndexError:
        print("perfectreport.py -h\nperfectreport.py -f <start_time> <end_time> <clientname>..\nperfectreport.py -a <start_time> <end_time> <clientname>..\nperfectreport.py -uf <clientname>..")
