...
customer_list = ["ASG","AYI","ATA","CMN","CMD","DLD","EAP","KEV","LEN","MAL","MBV","MCB","MCL","MCN","MDT","MGE","MGX","MHB","MJJ","MJR","MKA","MMB","MME","MMT","MMV","MNI","MOO","MQT","MSC","MSU","MTA","MTF","MTY","MWP","MWT","MYM","QVM","YAA"] 
...
def exportAllExcel(customer_name,end_time):
    ...
    with connect_to_MYSQL("JERRY").cursor() as jerry_Cursor:
        ...
        sql_command = """select eib.BOX_NO BOX_NO,eob.REFERENCE_NO REFERENCE_NO, GROUP_CONCAT(DISTINCT eim.PRODUCT_NAME SEPARATOR '，') PRODUCT_NAME, GROUP_CONCAT(DISTINCT eim.BRAND SEPARATOR '，') BRAND,
                         eib.WEIGHT DECLARE_WEIGHT,eib.REAL_WEIGHT REAL_WEIGHT ,
                            ...
                            where ecb.USERNAME = '{}'
and eib.CREATED_DATE between '{}' and '{}'
and eib.ENABLED_BOX = 'Y' GROUP BY eib.BOX_NO;""".format(customer_name,start_time,end_time)
        jerry_Cursor.execute(sql_command)
...
MIN(CASE WHEN cd.cinfo = 'Shipment information processed.' THEN cd.cdate END) [SHIPMENT],
 ...
 from client_rec cr left join check_detail cd on cr.irid = cd.irid where cr.cnum in ('{}')
group by cr.cnum;
    ...
    return outputfile("AllMel",customer_name+".xlsx",result_data,start_time,end_time)
...
if __name__ == '__main__':
    ...
    for customer in customer_list:
        file.append(exportAllExcel(customer,end_time));
    ...
    subject = '墨尔本自动生成报表'
    text = '这是自动生成的墨尔本报表，有问题请联系小白bain.bai@ewe.com.au'
